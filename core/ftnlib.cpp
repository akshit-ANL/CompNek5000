...
extern "C" {
    ...
    void getmolecularweights_(doublereal* WT)
    {
        return _gas->getMolecularWeights(WT);
    }
...
*/
    ...
    void setstate_uv_(double* u, double* v)
    {
        try {
            _gas->setState_UV(*u, *v);
        } catch (CanteraError& err) {
            handleError(err);
        }
    }

    void setstate_td_(double* T, double* rho)
    {
        try {
            _gas->setState_TD(*T, *rho);
        } catch (CanteraError& err) {
            handleError(err);
        }
    }

    void setmassfractions_(double* Y)
    {
        try {
    ...
    void getpartialmolarenthalpies_(doublereal* H)
    {
        _gas->getPartialMolarEnthalpies(H);
    }

    void getpartialmolarcp_(doublereal* CP)
    {
        _gas->getPartialMolarCp(CP);
    }
